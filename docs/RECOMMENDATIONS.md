...
| Tier agent tooling by standing context cost: CLI on PATH + a thin skill first (gh, curl, agent-browser), project-scoped MCP (`.mcp.json` + `opencode.json` in the repo) for domain servers like mcp-nixos, global MCP only for broadly useful servers (context7, filesystem). | MCP tool schemas load into every session for clients that read them upfront (opencode, Copilot), whether used or not; CLIs cost nothing until invoked and project scope confines the cost to repos that benefit. Skills cost one description line until triggered. |
