...
| `attic` | `enable`, `port`, `apiEndpoint`, `s3.endpoint`, `s3.bucket`, `s3.region`, `retentionPeriod`, `openFirewall`, `secrets.environment`, `environmentFile` | self-hosted Nix binary cache server (`services.atticd`, monolithic) with NARs in an S3 bucket (TrueNAS MinIO); one sops env file carries the RS256 JWT secret and the `AWS_*` key pair; pair with a traefik `routes` entry (`backend = "host"`, `auth = "none"`) for TLS | <https://docs.attic.rs/> |
