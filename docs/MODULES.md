...
| Module | Main options | Notes | Upstream refs |
| --- | --- | --- | --- |
| `flatpak` | `enable`, `desktop`, `browsers`, `cad`, `electronics`, `gaming`, `extraPackages`, `unprivilegedRuntimeInstall` | wraps `nix-flatpak` with category toggles; every enabled category is concatenated with the host's `extraPackages` and deduped, so a host adds to the set rather than replacing it; `desktop` (Flatseal, LibreOffice, VLC, Moonlight, Warehouse) defaults to `packages.includeDesktop` and `gaming` (SGDBoop, Lutris) to `features.gaming.enable`, so either can be forced on or off per host; the root timer only updates the declared packages, so a desktop updater (KDE Discover) still handles the rest — `unprivilegedRuntimeInstall` adds a polkit rule granting `org.freedesktop.Flatpak.runtime-install` to active local sessions so a non-wheel user is not prompted for an admin password when an update pulls a new runtime branch | <https://mynixos.com/search?q=services.flatpak> |
...
| `compose` | `projects.<name>` (`files`, `networks`, `envFile`, `prepare`, `runtimeDirectory`, `timeout`, `extraUpFlags`, `restartTriggers`) | shared scaffolding for compose projects as boot-time systemd oneshots — owns the unit ordering/lifecycle invariants and a `<name>-compose` day-2 wrapper; traefik, litellm, comfyui, and odysseus declare their projects through it. Units retry on failure (`on-failure`, 30s, bounded by a 5-in-600s start limit): a project that builds or pulls needs a registry, and no ordering edge expresses “the internet is up” — without the retry a boot-time DNS race leaves the containers running under docker's own restart policy while the owning unit sits failed | <https://docs.docker.com/compose/> |
...
the NVIDIA entries. On a host with more than one GPU set `transcodingDevice`
to the render node with the video driver (`ls -l /dev/dri/by-path`): left on
auto, Immich takes the highest-numbered node, and a wrong one fails every
transcode over to software ffmpeg.

**Sizing.** `memory.server` caps the server container, which is the API,
